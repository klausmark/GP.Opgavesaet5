...
namespace GP.Opgavesaet5.Opgave51.Model
{
    public class CprParser
    {
        ...
        private void DoRegexMatch()
        {
            ...
            _day = int.Parse(_match.Groups[1].Value);
            _month = int.Parse(_match.Groups[2].Value);
            _year = int.Parse(_match.Groups[3].Value);
            _lastFourDigits = int.Parse(_match.Groups[4].Value);
